...
class GenshinInfoManager:
    ...
    async def update_from_enka(self) -> str:
        ...
        if not data:
            log.info(f'原神信息: 无法获取到 {self.uid} 的数据，可能是 Enka.Network 接口服务出现问题')
        ...
        log.info(f'原神信息: ➤UID - {self.uid} 更新Enka成功')
    ...
    async def update_talent(self) -> str:
        ...
        for character in characters:
            ...
            if not isinstance(data, dict):
                ...
                log.info(f'原神信息: 更新 {self.uid} 的 {character.name} 角色天赋时出错，消息为 {data["message"]}')
                return data['message']
        ...
        log.info(f'原神信息: ➤UID {self.uid} 更新角色天赋成功')
        return '更新成功'
    ...
    async def update_from_mihoyo(self) -> str:
        ...
        if not isinstance(chara_data, dict):
            ...
            log.info(f'原神信息: 更新 {self.uid} 的玩家角色数据时出错，消息为 {chara_data["message"]}')
        ...
        log.info(f'原神信息: ➤UID {self.uid} 更新玩家信息成功')
        return '更新成功'
    ...
    async def update_abyss_info(self, abyss_index: int) -> str:
        ...
        log.info(f'原神信息: ➤UID {self.uid} 更新深渊信息成功')
        return '更新成功'
        ...
        if data_source == 'enka':
            ...
            if not character or character.update_time < (datetime.datetime.now() - datetime.timedelta(hours=6)).replace(
                ...
                if character := await Character.get_or_none(**query, data_source='enka'):
                    ...
                    log.info(f'原神角色面板: ➤➤角色: {name or character_id} 不在游戏内展示柜中，更新失败')
            else:
                log.info(f'原神角色面板: ➤➤角色: {name or character_id} 数据更新成功')
            return character
