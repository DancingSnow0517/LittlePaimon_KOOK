...
from ....database.models.cookie import PublicCookie, PrivateCookie
...
async def get_blueprint_data(share_code: int, user_id: Optional[str]) -> Tuple[
    Union[List[Item], str, int, None], Optional[str]]:
    ...
    for cookie in cookies:
        ...
        resp = await requests.get(CODE_API,
                                  params={
        ...
        if data['retcode'] == 0:
            ...
            log.info(f'尘歌壶摹本: 摹数 {share_code} 数据获取成功')
            return items, cookie.cookie
        elif data['retcode'] != -100:
            log.info(f'尘歌壶摹本: 摹数 {share_code} 数据获取失败，{data["message"] or str(data["retcode"])}')
            return data['message'] or str(data['retcode']), None
    log.info('尘歌壶摹本', f'摹数<m>{share_code}</m>数据<r>获取失败，没有可用的Cookie</r>')
    return None, None
...
async def get_blueprint_material_data(items: List[Item], cookie: Optional[str]) -> Union[List[Item], str, int, None]:
    ...
    resp = await requests.post(MATERIAL_API,
                               json=item_list,
                               headers=headers)
    data = resp.json()
    if data['retcode'] == 0:
