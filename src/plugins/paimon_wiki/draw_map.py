...
async def draw_map(name: str, map_: str):
    ...
    if resources := list(filter(lambda x: x.name == name, [child for label in labels for child in label.children])):
        ...
        return CardMessage(Card(Section(Kmarkdown(f'未查找到材料 **{name}**')), theme=ThemeTypes.NONE))
    ...
    if not (points := utils.convert_pos(utils.get_points_by_id(resource.id, points), maps.detail.origin)):
        return CardMessage(
            Card(Section(Kmarkdown(f'在{map_}上未查找到材料{name}，请尝试其他地图')), theme=ThemeTypes.NONE))
    ...
    if des:
        total_img.text_box(des.replace('\n', '^'), (482, 1010), (281, 520), fm.get('SourceHanSansCN-Bold.otf', 30),
                           '#3c3c3c')
    total_img.text('CREATED BY LITTLEPAIMON', (0, total_img.width), total_img.height - 45,
