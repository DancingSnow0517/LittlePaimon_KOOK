...
async def draw_chara_card(info: Character) -> PMImage:
    ...
    # 角色名
    img.text(info.name, left_point, 161, fm.get('hywh', 24), '#252525')
    # 等级
    img.text(f'LV{info.level}', left_point + name_length + 10, 163, fm.get('bahnschrift_bold', 24, 'Bold'),
             '#593d99' if info.rarity == 4 else '#964e2e')
    # 元素图标
    img.paste(await load_image(ICON / f'{info.element}.png'), (9, 12))
    # 命座图标
    img.paste(await load_image(ICON / f'命之座{len(info.constellation)}.png'), (197, 11))
    # 好感度
    img.paste(await load_image(ICON / f'好感度{info.fetter}.png', size=0.6), (189, 118))
    # 武器图标
    weapon = await draw_weapon_icon(info.weapon, (60, 60))
    img.paste(weapon, (14, 196))
    # 武器名
    img.text(info.weapon.name, 79, 195, fm.get('hywh', 24), '#33231a')
    # 武器等级
    img.text(f'Lv.{info.weapon.level}', 80, 227, fm.get('bahnschrift_bold', 24, 'Bold'),
             weapon_affix_color[info.weapon.affix_level])
    # 武器精炼图标
    img.paste(await load_image(ICON / f'精炼{info.weapon.affix_level}.png'), (150, 227))
    # 圣遗物图标
    ...
    img.paste(artifact_bg, (14, 265))
    if len(info.suit) == 2:
        if info.suit[0] == info.suit[1]:
            img.paste(await load_image(ARTIFACT_ICON / f'{info.suit[0][1]}.png', mode='RGBA', size=(60, 60)),
                      (14, 265))
            suit_name = f'{info.suit[0][0][:2]}四件套'
        else:
            img.paste(await load_image(ARTIFACT_ICON / f'{info.suit[0][1]}.png', mode='RGBA', size=(42, 42)),
                      (14, 265))
            img.paste(await load_image(ARTIFACT_ICON / f'{info.suit[1][1]}.png', mode='RGBA', size=(42, 42)),
                      (36, 283))
        ...
        img.paste(await load_image(ARTIFACT_ICON / f'{info.suit[0][1]}.png', mode='RGBA', size=(42, 42)),
                  (14, 265))
        img.paste(await load_image(ARTIFACT_ICON / '不知名花.png', mode='RGBA', size=(32, 32)),
                  (40, 290))
    elif len(info.artifacts):
        suit_name = '散搭圣遗物'
        img.paste(await load_image(ARTIFACT_ICON / '不知名花.png', mode='RGBA', size=(32, 32)),
                  (18, 272))
        img.paste(await load_image(ARTIFACT_ICON / '不知名毛.png', mode='RGBA', size=(32, 32)),
                  (40, 290))
    else:
        suit_name = '未装备圣遗物'
        img.paste(await load_image(ARTIFACT_ICON / '不知名花.png', mode='RGBA', size=(50, 50)),
                  (19, 271))
    # 圣遗物名
    img.text(suit_name, 82, 265, fm.get('hywh', 24), '#252525')
    # 圣遗物评分
    ...
    if score is not None:
        ...
        img.text(str(score), 81, 296, fm.get('bahnschrift_bold', 24, 'Bold'), artifact_score_color[rank])
        img.paste(await load_image(ICON / f'rank{rank}.png'), (150, 297))
    else:
        img.text('---', 81, 296, fm.get('hywh', 24), '#33231a')
    # 天赋等级色块
    if not info.talents:
        img.draw_rounded_rectangle2((4, 339), (228, 69), 15, '#d5f2b6', ['ll', 'lr'])
        img.text('未获取等级', (4, 232), (339, 408), fm.get('hywh', 24), '#6d993d', 'center')
    else:
        for i in range(3):
            bg_color = talent_color[info.talents[i].level - 1][0]
            text_color = talent_color[info.talents[i].level - 1][1]
            if i == 0:
                img.draw_rounded_rectangle2((4, 339), (76, 69), 15, bg_color, ['ll'])
                img.text(str(info.talents[i].level), (4, 80), (339, 408), fm.get('bahnschrift_bold', 48, 'Bold'),
                         text_color, 'center')
            elif i == 1:
                img.draw_rounded_rectangle2((81, 339), (76, 69), 15, bg_color, [])
                img.text(str(info.talents[i].level), (81, 157), (339, 408),
                         fm.get('bahnschrift_bold', 48, 'Bold'),
                         text_color, 'center')
            else:
                if info.name in ['神里绫华', '莫娜']:
                    i += 1
                img.draw_rounded_rectangle2((156, 339), (76, 69), 15, bg_color, ['lr'])
                img.text(str(info.talents[i].level), (157, 233), (339, 408),
                         fm.get('bahnschrift_bold', 48, 'Bold'),
                         text_color, 'center')
        img.draw_line((80, 339), (80, 413), '#593d99' if info.rarity == 4 else '#964e2e', 2)
        img.draw_line((157, 339), (157, 413), '#593d99' if info.rarity == 4 else '#964e2e', 2)
    img.draw_line((0, 339), (237, 339), '#593d99' if info.rarity == 4 else '#964e2e', 2)
    return img


async def draw_chara_bag(player: Player, info: PlayerInfo, characters: List[Character], avatar_url: str):
    characters = sort_characters(characters)
    # 确定角色行数，4个为一行
    row = math.ceil(len(characters) / 4)
    img = PMImage(await load_image(RESOURCES / 'bg.png'))
    img.stretch((255, 1534), 428 * row - 21, 'height')
    # 头像
    avatar = await get_avatar(avatar_url, (108, 108))
    img.paste(avatar, (38, 45))
    # 昵称
    img.text(info.nickname, 166, 49, fm.get('hywh', 48), '#252525')
    # 签名和uid
    if info.signature:
        ...
        img.text(f'UID{player.uid}', 165, 103, fm.get('bahnschrift_regular', 48, 'Regular'), '#252525')

    # 角色数量和logo
    img.text(f'共计{len(characters)}名角色', 237, 190, fm.get('hywh', 30), '#252525')
    img.text(f'CREATED BY LITTLEPAIMON AT {datetime.datetime.now().strftime("%m-%d %H:%M")}', 1031, 194,
             fm.get('bahnschrift_bold', 30, 'Bold'), '#252525', 'right')

    # 角色列表
    for i in range(len(characters)):
        img.paste(await draw_chara_card(characters[i]), (38 + 257 * (i % 4), 255 + 428 * (i // 4)))
