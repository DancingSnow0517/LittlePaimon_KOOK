...
from ...utils.image_util import font_manager, PMImage
...
def get_damage_multipiler(info: Character) -> Optional[Dict[str, any]]:
    ...
    if info.name == '诺艾尔':
        ...
        return {
            ...
            'T:普攻治疗量/概率': str(
                int((float(ez[0].replace('%防御力', '')) / 100.0 * info.prop.defense + float(ez[1])) * (
    ...
    if info.name == '荒泷一斗':
        return {
            ...
            'B:c6-暴击伤害-AZ': (0.7,),
            'B:l1-攻击力': (
                float(skill_data['最恶鬼王·一斗轰临！！']['数值']['攻击力提高'][level_q].replace('%防御力',
                                                                                               '')) / 100.0 * info.prop.defense,),
            'AZ-e岩:荒泷逆袈裟连斩': float(
    ...
    if info.name == '枫原万叶':
        ...
        return {
            ...
            'T:扩散伤害': int(
                upheaval_reaction(info.level, '扩散', info.prop.elemental_mastery,
                                  info.prop.reaction_coefficient['扩散']))
        }
    if info.name == '鹿野院平藏':
        e = skill_data['勠心拳']['数值']
    ...
    if info.name == '多莉':
        ...
        return {
            ...
            'T:大招持续治疗量': int(
                float(h[0].replace('%', '')) / 100.0 * info.prop.health + float(h[1]) * (1 + info.prop.healing_bonus)),
        }
    if info.name == '九条裟罗':
        atk = float(skill_data['鸦羽天狗霆雷召咒']['数值']['攻击力加成比例'][level_e].replace('%',
                                                                                              '')) / 100.0 * info.prop.base_attack
        return {
            'T:乌羽攻击加成': int(atk),
            'B:l0-攻击力-*': (atk,),
            'B:c6-暴击伤害-*': (0.6,),
...
async def draw_dmg_pic(dmg: Dict[str, Union[tuple, list]]) -> PMImage:
    ...
    img.draw_rounded_rectangle((0, 0, img.width, img.height), 10, (14, 41, 68, 115))
    img.draw_line((250, 0), (250, 1002), (255, 255, 255, 75), 2)
    img.draw_line((626, 0), (626, 60), (255, 255, 255, 75), 2)
    img.draw_line((0, 60), (1002, 60), (255, 255, 255, 75), 2)
    img.text('伤害计算', (0, 250), (0, 60), font_manager.get('hywh.ttf', 30), 'white', 'center')
    img.text('期望伤害', (250, 626), (0, 60), font_manager.get('hywh.ttf', 30), 'white', 'center')
    img.text('暴击伤害', (626, 1002), (0, 60), font_manager.get('hywh.ttf', 30), 'white', 'center')
    i = 1
    for describe, dmg_list in dmg.items():
        img.draw_line((0, 60 * i), (1002, 60 * i), (255, 255, 255, 75), 2)
        img.text(describe, (0, 250), (60 * i, 60 * (i + 1)), font_manager.get('hywh.ttf', 30), 'white', 'center')
        if len(dmg_list) == 1:
            if describe == '额外说明':
                img.text(dmg_list[0], (250, 1002), (60 * i, 60 * (i + 1)), font_manager.get('hywh.ttf', 30),
                         'white',
                         'center')
            else:
                img.text(dmg_list[0], (250, 1002), (60 * i, 60 * (i + 1)), font_manager.get('number.ttf', 30),
                         'white',
                         'center')
        else:
            img.draw_line((626, 60 * i), (626, 60 * (i + 1)), (255, 255, 255, 75), 2)
            img.text(dmg_list[0], (250, 626), (60 * i, 60 * (i + 1)), font_manager.get('number.ttf', 30), 'white',
                     'center')
            img.text(dmg_list[1], (626, 1002), (60 * i, 60 * (i + 1)), font_manager.get('number.ttf', 30), 'white',
                     'center')
        i += 1
