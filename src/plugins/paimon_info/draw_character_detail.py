...
async def draw_chara_detail(uid: str, info: Character):
    ...
    try:
        ...
        log.info(f'{info.name}的伤害计算制图失败：{e}')
        dmg_img = None
    if dmg_img:
        img.stretch((730, 1377), dmg_img.height + 667, 'height')
        img.paste(dmg_img, (42, 1820))
    ...
    if info.name not in ['荧', '空', '埃洛伊']:
        img.paste(await load_image(ENKA_RES / f'{info.region}.png', size=(108, 108)), (25, 25))

    img.text(f"UID{uid}", 160, 100, fm.get('number.ttf', 48))
    img.text(info.name, 45, 150, fm.get('优设标题黑.ttf', 72))
    name_length = img.text_length(info.name, fm.get('优设标题黑.ttf', 72))

    level_mask = await load_image(ENKA_RES / '等级遮罩.png')
    img.paste(level_mask, (45 + name_length + 25, 172))
    img.text(f'LV{info.level}',
             ...
             'center')
    # 属性值
    img.text('生命值', 59, 267, fm.get('hywh.ttf', 34))
    img.text(f"{info.prop.base_health}",
             450 - img.text_length(f"+{info.prop.extra_health}", fm.get('number.ttf', 34)) - 5,
             269,
             fm.get('number.ttf', 34),
             align='right')
    img.text(f"+{info.prop.extra_health}",
             450,
             269,
             fm.get('number.ttf', 34),
             '#59c538',
    ...
    img.text('攻击力', 59, 324, fm.get('hywh.ttf', 34))
    img.text(f"{info.prop.base_attack}",
             450 - img.text_length(f"+{info.prop.extra_attack}", fm.get('number.ttf', 34)) - 5,
             326,
             fm.get('number.ttf', 34),
             align='right')
    img.text(f"+{info.prop.extra_attack}",
             450,
             326,
             fm.get('number.ttf', 34),
             '#59c538',
    ...
    img.text('防御力', 59, 382, fm.get('hywh.ttf', 34))
    img.text(f"{info.prop.base_defense}",
             450 - img.text_length(f"+{info.prop.extra_defense}", fm.get('number.ttf', 34)) - 5,
             384,
             fm.get('number.ttf', 34),
             align='right')
    img.text(f"+{info.prop.extra_defense}",
             450,
             384,
             fm.get('number.ttf', 34),
             '#59c538',
    ...
    img.text('暴击率', 59, 441, fm.get('hywh.ttf', 34))
    img.text(f"{round(info.prop.crit_rate * 100, 1)}%",
             450,
             443,
             fm.get('number.ttf', 34),
             align='right')

    img.text('暴击伤害', 59, 498, fm.get('hywh.ttf', 34))
    img.text(f"{round(info.prop.crit_damage * 100, 1)}%",
             450,
             500,
             fm.get('number.ttf', 34),
             align='right')

    img.text('元素精通', 59, 556, fm.get('hywh.ttf', 34))
    img.text(str(int(info.prop.elemental_mastery)),
             450,
             558,
             fm.get('number.ttf', 34),
             align='right')

    img.text('元素充能效率', 59, 615, fm.get('hywh.ttf', 34))
    img.text(f"{round(info.prop.elemental_efficiency * 100, 1)}%",
             450,
             617,
             fm.get('number.ttf', 34),
             align='right')
    ...
    for e, v in info.prop.dmg_bonus.items():
        if v >= max_element[1]:
            max_element = e, v
    img.text(f'{max_element[0]}伤害加成' if max_element[0] == '物理' else f'{max_element[0]}元素伤害加成',
             59,
             674,
             fm.get('hywh.ttf', 34))
    img.text(f"{round(max_element[1] * 100, 1)}%",
             450,
             676,
             fm.get('number.ttf', 34),
             align='right')
    ...
    if info.name in ['神里绫华', '莫娜']:
        info.talents.pop(2)
    for i in range(3):
        img.paste(base_icon.resize((132, 142)), (551 + i * 176, 633))
        img.text(str(info.talents[i].level),
                 (517 + 176 * i, 559 + 176 * i),
                 690,
                 fm.get('number.ttf', 34),
                 '#0e2944',
                 'center')
        img.paste(await load_image(TALENT_ICON / f'{info.talents[i].icon}.png', size=(57, 57), mode='RGBA'),
                  (588 + i * 176, 679))
    ...
    for con in info.constellation:
        img.paste(base_icon.resize((83, 90)), (510 + t * 84, 805))
        con_icon = await load_image(TALENT_ICON / f'{con.icon}.png', size=(45, 45), mode='RGBA')
        img.paste(con_icon, (529 + t * 84, 828))
        t += 1
    for t2 in range(t, 6):
        img.paste(base_icon_grey.resize((83, 90)), (510 + t2 * 84, 805))
        img.paste(lock, (530 + t2 * 84, 828))

    # 武器
    weapon_bg = await load_image(ICON / f'star{info.weapon.rarity}.png', size=(150, 150))
    img.paste(weapon_bg, (59, 757))
    weapon_icon = await load_image(WEAPON_ICON / f'{info.weapon.icon}.png', size=(150, 150), mode='RGBA')
    img.paste(weapon_icon, (59, 757))
    img.text(info.weapon.name, 254, 759, fm.get('hywh.ttf', 34))

    star = await load_image(ENKA_RES / 'star.png')
    for i in range(info.weapon.rarity):
        img.paste(star, (254 + i * 30, 798))
    img.text(f'LV{info.weapon.level}',
             (254, 254 + 98),
             (834, 864),
             fm.get('number.ttf', 27),
             '#0e2944',
             'center')
    img.text(f'精炼{info.weapon.affix_level}阶', 254, 879, fm.get('hywh.ttf', 34))
    ...
    for artifact in info.artifacts:
        ...
        img.paste(artifact_bg, (216 + w, 70 + h))
        a_icon = await load_image(ARTIFACT_ICON / f'{artifact.icon}.png', size=(93, 93), mode='RGBA')
        img.paste(a_icon, (217 + w, 70 + h))
        img.text(artifact.name, 22 + w, 24 + h, fm.get('hywh.ttf', 36))
        ...
        img.text(f'{rank}-{value}', 22 + w, 66 + h, fm.get('number.ttf', 28), '#ffde6b')
        img.paste(level_mask.resize((98, 30)), (21 + w, 97 + h))
        img.text(f"+{artifact.level}",
                 (21 + w, 21 + w + 98),
                 99 + h,
                 fm.get('number.ttf', 27),
                 'black',
                 'center')
        img.text(artifact.main_property.name, 21 + w, 134 + h, fm.get('hywh.ttf', 25))
        ...
        img.text(value_text, 21 + w, 168 + h, font=fm.get('number.ttf', 48))
    ...
    if len(total_rank) == 3:
        img.paste(rank_icon, (65, 964))
        img.paste(rank_icon, (115, 964))
        img.paste(rank_icon, (165, 964))
        img.text(str(round(total_score, 1)), 220, 974, font=fm.get('number.ttf', 60))
    elif len(total_rank) == 2:
        img.paste(rank_icon, (95, 964))
        img.paste(rank_icon, (145, 964))
        img.text(str(round(total_score, 1)), 205, 974, font=fm.get('number.ttf', 60))
    else:
        img.paste(rank_icon, (113, 964))
        img.text(str(round(total_score, 1)), 187, 974, font=fm.get('number.ttf', 60))

    # 圣遗物套装
    suit = GenshinTools.get_artifact_suit(info.artifacts)
    if not suit:
        img.text('未激活套装', 154, 1168, font=fm.get('hywh.ttf', 36))
        img.text('未激活套装', 154, 1292, font=fm.get('hywh.ttf', 36))
    elif len(suit) == 1:
        artifact1 = await load_image(ARTIFACT_ICON / f'{suit[0][1]}.png', size=(110, 110), mode='RGBA')
        img.paste(artifact1, (46, 1130))
        img.text(f'{suit[0][0][:2]}二件套', 154, 1168, font=fm.get('hywh.ttf', 36))
        img.text('未激活套装', 154, 1292, font=fm.get('hywh.ttf', 36))
    else:
        if suit[0][0] == suit[1][0]:
            artifact2 = artifact1 = await load_image(ARTIFACT_ICON / f'{suit[0][1]}.png', size=(110, 110), mode='RGBA')
            img.text(f'{suit[0][0][:2]}四件套', 154, 1168, font=fm.get('hywh.ttf', 36))
            img.text(f'{suit[0][0][:2]}四件套', 154, 1292, font=fm.get('hywh.ttf', 36))
        else:
            artifact1 = await load_image(ARTIFACT_ICON / f'{suit[0][1]}.png', size=(110, 110), mode='RGBA')
            artifact2 = await load_image(ARTIFACT_ICON / f'{suit[1][1]}.png', size=(110, 110), mode='RGBA')
            img.text(f'{suit[0][0][:2]}两件套', 154, 1168, font=fm.get('hywh.ttf', 36))
            img.text(f'{suit[1][0][:2]}两件套', 154, 1292, font=fm.get('hywh.ttf', 36))
        img.paste(artifact1, (46, 1130))
        img.paste(artifact2, (46, 1255))

    # 立绘
    img.text(f'更新于{info.update_time.strftime("%m-%d %H:%M")}',
             (0, 1080),
