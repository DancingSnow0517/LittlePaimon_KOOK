...
async def get_voice(text: str, char: str, noise: float = 0.667, length: float = 1.2, format: str = 'mp3') -> bytes:
    """
    得到 TTS 语音
    :param text: 语音文字
    :param char: 语音使用的角色
    ...
    res = await requests.get(
        f'http://245671.proxy.nscc-gz.cn:8888//?text={text}&speaker={char}&noise={noise}&length={length}&format={format}')
    return res.read()
