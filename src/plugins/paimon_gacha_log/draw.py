...
from .models import FiveStarItem, FourStarItem, GachaLogInfo
...
async def get_avatar(avatar_url: str, size: Tuple[int, int] = (146, 146)) -> PMImage:
    try:
...
async def detail_avatar(info: FiveStarItem):
    ...
    bg.draw_rounded_rectangle2((76, 3), (58, 33), 10, color[0], ['ur', 'll'])
    bg.text(str(info.count), (76, 134), 3, fm.get('bahnschrift_regular', 40, 'Bold'), color[1], 'center')
    return bg


async def draw_pool_detail(pool_name: str, data: List[FiveStarItem], total_count: int, not_out: int) -> Optional[PMImage]:
    if not data:
        return None
    ...
    # 橙线
    img.paste(await load_image(RESOURCE_BASE_PATH / 'general' / 'line.png'), (0, 0))
    pool_type = pool_name[:2]
    img.text(f'{pool_type}卡池', 25, 11, fm.get('hywh', 30), 'white')
    # 数据
    img.text('平均出货', 174, 137, fm.get('hywh', 24), (24, 24, 24, 102))
    ave = round((total_count - not_out) / len(data), 2)
    img.text(str(ave), (176, 270), 84,
             fm.get('bahnschrift_regular', 48, 'Regular'),
             '#252525', 'center')
    img.text('总抽卡数', 372, 137, fm.get('hywh', 24), (24, 24, 24, 102))
    img.text(str(total_count), (372, 467), 84, fm.get('bahnschrift_regular', 48, 'Regular'),
             '#252525', 'center')
    img.text('未出五星', 562, 137, fm.get('hywh', 24), (24, 24, 24, 102))
    img.text(str(not_out), (562, 655), 84, fm.get('bahnschrift_regular', 48, 'Regular'),
             '#252525', 'center')
    lucky = '欧' if ave <= (45 if pool_type == '武器' else 50) else '吉' if ave <= (
        55 if pool_type == '武器' else 60) else '中' if ave <= (65 if pool_type == '武器' else 70) else '非'
    img.paste(await load_image(RESOURCE_BASE_PATH / 'gacha_log' / f'{lucky}{random.randint(1, 3)}.png'), (753, 68))
    # 折线图
    if len(data) > 3:
        last_point = None
        img.paste(await load_image(RESOURCE_BASE_PATH / 'gacha_log' / 'broken_line_bg.png'), (1, 181))
        i = 0
        for chara in data[-10:]:
            height = int(583 - (chara.count / 90) * 340)
            if last_point:
                img.draw_line(last_point, (line_point[i], height), '#ff6f30', 4)
        ...
        for chara in data[-10:]:
            ...
            img.paste(await small_avatar(chara), (point, height - 23))
            img.text(str(chara.count), (point, point + 44), height - 48, fm.get('bahnschrift_regular', 30,
                                                                                'Regular'), '#040404', 'center')
            i += 1
    # 详细数据统计
    chara_bg = PMImage(await load_image(RESOURCE_BASE_PATH / 'gacha_log' / 'detail_bg.png'))
    chara_bg.stretch((47, chara_bg.height - 20), 192 + 192 * (len(data) // 6), 'height')
    img.paste(chara_bg, (1, 655 if len(data) > 3 else 181))
    for i in range(len(data)):
        img.paste(await detail_avatar(data[i]), (18 + i % 6 * 163, (708 if len(data) > 3 else 234) + i // 6 * 192))
    return img
...
async def draw_four_star(info: FourStarItem) -> PMImage:
    ...
    img.to_circle('circle')
    bg.paste(img, (34, 26))
    bg.text(info.name, (0, bg.width), 163, fm.get('hywh', 24), '#221a33', 'center')
    bg.text(str(info.num['角色祈愿']), (4, 64), 209, fm.get('bahnschrift_regular', 36, 'Bold'), '#3d6e99',
            'center')
    bg.text(str(info.num['武器祈愿']), (65, 125), 209, fm.get('bahnschrift_regular', 36, 'Bold'), '#593d99',
            'center')
    bg.text(str(info.num['常驻祈愿'] + info.num['新手祈愿']), (126, 186), 209,
            fm.get('bahnschrift_regular', 36, 'Bold'),
            '#3a9381',
            'center')
    return bg
...
async def draw_four_star_detail(data: List[FourStarItem]):
    ...
    bg.paste(bar, (0, 0))
    for i in range(len(data)):
        bg.paste(await draw_four_star(data[i]), (i % 5 * 204, 105 + i // 5 * 260))
    return bg


async def draw_gacha_log(avatar_url: str, uid: str, nickname: Optional[str], signature: Optional[str],
                         gacha_log: GachaLogInfo):
    ...
    # 头像
    avatar = await get_avatar(avatar_url, (108, 108))
    img.paste(avatar, (38, 45))
    # 昵称
    img.text(nickname, 166, 49, fm.get('hywh', 48), '#252525')
    # 签名和uid
    if signature:
        img.text(signature, 165, 116, fm.get('hywh', 32), '#252525')
        nickname_length = img.text_length(nickname, fm.get('hywh', 48))
        img.text(f'UID{uid}', 166 + nickname_length + 36, 58,
                 fm.get('bahnschrift_regular', 48, 'Regular'),
                 '#252525')
    else:
        img.text(f'UID{uid}', 165, 103, fm.get('bahnschrift_regular', 48, 'Regular'), '#252525')

    data5, data4, data_not = gacha_log.get_statistics()
    # 数据总览
    img.paste(line, (36, 181))
    img.text('数据总览', 60, 192, fm.get('hywh', 30), 'white')
    img.text(f'CREATED BY LITTLEPAIMON AT {datetime.datetime.now().strftime("%m-%d %H:%M")}', 1025, 196,
    ...
    img.text('平均出货', 209, 335, fm.get('hywh', 24), (24, 24, 24, 102))
    img.text(str(five_star_average), (211, 305), 286, fm.get('bahnschrift_regular', 48), '#040404', 'center')
    img.text('总抽卡数', 407, 335, fm.get('hywh', 24), (24, 24, 24, 102))
    img.text(str(total_gacha_count), (408, 503), 286, fm.get('bahnschrift_regular', 48), '#040404', 'center')
    img.text('总计出金', 597, 335, fm.get('hywh', 24), (24, 24, 24, 102))
    img.text(str(total_five_star_count), (598, 694), 286, fm.get('bahnschrift_regular', 48), '#040404', 'center')
    lucky = '欧' if five_star_average <= 50 else '吉' if five_star_average <= 60 else '中' if five_star_average <= 70 else '非'
    img.paste(await load_image(RESOURCE_BASE_PATH / 'gacha_log' / f'{lucky}{random.randint(1, 3)}.png'), (788, 271))
    ...
    if total_five_star_count:
        ...
        for per in pers:
            if per >= 3:
                img.draw_rectangle((now_used_width, 399, now_used_width + int(per / 100 * 967), 446),
                                   bar_color[i][0])
                if per >= 10:
                    img.text(f'{per}%', now_used_width + 18, 410, fm.get('bahnschrift_regular', 30, 'Bold'),
                             bar_color[i][1])
                now_used_width += int(per / 100 * 967)
            i += 1
        img.paste(await load_image(RESOURCE_BASE_PATH / 'gacha_log' / 'text.png'), (484, 464))
        now_height = 525
        for pool_name, data in data5.items():
            pool_detail_img = await draw_pool_detail(pool_name, data, len(gacha_log.item_list[pool_name]),
                                                     data_not[pool_name])
            if pool_detail_img:
                img.paste(pool_detail_img, (36, now_height))
                now_height += pool_detail_img.height
        now_height += 10
        img.paste(four_star_detail, (36, now_height))
        now_height += four_star_detail.height + 30

        img.crop((0, 0, 1080, now_height))
        bg.stretch((50, bg.height - 50), now_height - 100, 'height')

    else:
        img.paste(four_star_detail, (36, 410))
        img.crop((0, 0, 1080, 410 + four_star_detail.height + 30))
        bg.stretch((50, bg.height - 50), img.height - 100, 'height')
    bg.paste(img, (0, 0))
    return bg
