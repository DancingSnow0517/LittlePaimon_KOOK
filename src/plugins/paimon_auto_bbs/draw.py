...
async def draw_result(group_id: int, data: list):
    ...
    img.stretch((50, img.height - 50), total_height, 'height')
    img.stretch((60, 415), 0, 'width')

    text_width = [70, 290, 510]

    img.text('米游社自动签到结果', 50, 30, fm.get('优设标题黑', 60), '#252525')
    img.text(str(group_id), 50, 85, fm.get('优设标题黑', 48), '#252525')
    img.text(f'本群已开启自动签到共{len(data)}人', 50, 145, fm.get('优设标题黑', 36), '#252525')

    img.paste(await load_image(RESOURCE_BASE_PATH / 'icon' / 'success.png', size=(50, 50)), (50, 220))
    img.text('签到成功：', 110, 230, fm.get('优设标题黑', 36), '#00921b')
    now_height = 270
    for reward in same_reward:
        img.text(f'{reward}', (0, img.width), now_height, fm.get('优设标题黑', 30), '#252525', 'center')
        now_height += 45
        i = 0
        for uid in same_reward[reward]:
            img.text(str(uid), text_width[i % 3], now_height + (i // 3) * 45, fm.get('优设标题黑', 25), '#252525')
            i += 1
        now_height += math.ceil(len(same_reward[reward]) / 3) * 45
    now_height += 20
    img.paste(await load_image(RESOURCE_BASE_PATH / 'icon' / 'fail.png', size=(50, 50)), (50, now_height))
    img.text('签到失败(CK失效或出现验证码)：', 110, now_height + 10, fm.get('优设标题黑', 36), '#d60623')
    now_height += 60
    i = 0
    for uid in fail_list:
        img.text(str(uid), text_width[i % 3], now_height + (i // 3) * 45, fm.get('优设标题黑', 25), '#252525')
        i += 1
    img.text('Powered by LittlePaimon', (0, img.width), img.height - 65, fm.get('优设标题黑', 30), '#252525', 'center')
