...
async def check_token(uuid: str, cookie: str):
    ...
    req = await requests.post(f'{host}hk4e_cg_cn/gamer/api/login', headers=headers)
...
async def get_info(uuid: str, cookie: str):
    headers = get_header(uuid, cookie)
    req = await requests.get(f'{host}hk4e_cg_cn/wallet/wallet/get', headers=headers)
...
async def get_announcement(uuid: str, cookie: str):
    headers = get_header(uuid, cookie)
    req = await requests.get(f'{host}hk4e_cg_cn/gamer/api/getAnnouncementInfo', headers=headers)
